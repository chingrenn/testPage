...
CloudflareApps.siteId = "b661dfc44e36f0de323e547650e691c6";
...
CloudflareApps.installs["awkrpSsB-9do"] = {
    appId: "lMxPPXVOqmoE",
    scope: {}
};;
CloudflareApps.installs["awkrpSsB-9do"].options = {
    "id": "UA-61945105-1",
    "social": true
};;
if (CloudflareApps.matchPage(CloudflareApps.installs['awkrpSsB-9do'].URLPatterns)) {
...
if (CloudflareApps.matchPage(CloudflareApps.installs['awkrpSsB-9do'].URLPatterns)) {
    (function() {
        var options = CloudflareApps.installs['awkrpSsB-9do'].options
            ...
            queue(function() {
                ...
                if ('FB' in window && 'Event' in FB && 'subscribe' in window.FB.Event) {
                    FB.Event.subscribe('edge.create', function(targetUrl) {
                        window.ga('send', 'social', 'facebook', 'like', {
                            page: targetUrl
                        })
                    })
                    FB.Event.subscribe('edge.remove', function(targetUrl) {
                        window.ga('send', 'social', 'facebook', 'unlike', {
                            page: targetUrl
                        })
                    })
                    FB.Event.subscribe('message.send', function(targetUrl) {
                        window.ga('send', 'social', 'facebook', 'send', {
                            page: targetUrl
                        })
                    })
                }
                if ('twttr' in window && 'events' in twttr && 'bind' in twttr.events) {
                    twttr.events.bind('tweet', function(event) {
                        if (event) {
                            ...
                            window.ga('send', 'social', 'twitter', 'tweet', {
                                page: targetUrl
                            })
                        }
                    })
